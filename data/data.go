...
import (
	"encoding/json"
	"fmt"
	"octo-command/apps/octo/infrastructure/datastore"
...
const (
	...
	CONFIG_STORE         string = ".store"
	CONFIG_RESOURCE      string = "config"
)

type StorageDataService struct {
	db datastore.DatabasePort
}

func (odp *ports.OctoDataPort) GetConfig() (*models.Config, error) {
	...
	rec := models.Config{}
	err = db.Read(CONFIG_STORE, CONFIG_RESOURCE, rec)
	if err != nil {
		return nil, err
	}
